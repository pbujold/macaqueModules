...
#                fullMags = np.concatenate(Xs[:,0::2])
                fullMags = np.hstack((Xs[:,0], Xs[:,2][(Xs[:,2] != 0)], Xs[:,4],  Xs[:,6][(Xs[:,6] != 0)]))     
                
            if  min(fullMags) < 0.15:
                newX = (Xs[:,0::2] - 0) / (max(fullMags) -0)
            else:
                newX = (Xs[:,0::2] - min(fullMags)) / (max(fullMags) - min(fullMags))
    ...
    if dynamic == False:
        if len(sig.parameters) > 1:
            if sfm_function.lower() == 'probit':
                SFM = lambda X, params : norm.cdf(x=(VALUE( X[:,:4] , params[1:]) - VALUE(X[:,4:], params[1:])), loc=0, scale=1/params[0])
                sfm_parameters = ['noise']
                sfm0 = [1]
            elif sfm_function.lower() == 'logit':
                SFM = lambda X, params : 1 / (1 + np.exp( -params[0] * ( (VALUE( X[:,:4] , params[1:]) - VALUE(X[:,4:], params[1:])) )  ))
                sfm_parameters = ['noise']
                sfm0 = [1]
                ...
                SFM = lambda X, params : 1 / (1 + np.exp( ( (VALUE( X[:,:4] , params[1:]) - VALUE(X[:,4:], params[1:])) ) / np.sqrt(VARn21)  ))
        else:
            if sfm_function.lower() == 'probit':
                SFM = lambda X, params : norm.cdf(x=(VALUE( X[:,:4]) - VALUE(X[:,4:])), loc=0, scale=1/params[0])
                sfm_parameters = ['noise']
                sfm0 = [1]
            elif sfm_function.lower() == 'logit':
                SFM = lambda X, params : 1 / (1 + np.exp( -params[0] *  (VALUE( X[:,:4] ) - VALUE(X[:,4:]))   ))
                sfm_parameters = ['noise']
                sfm0 = [1]
            elif sfm_function.lower() == 'random':
                SFM = lambda y, params : np.random.rand(len(y))
                ...
                VARn21 = np.exp(s+(g*time))
                SFM = lambda X, params : 1 / (1 + np.exp( ( (VALUE( X[:,:4] , ) - VALUE(X[:,4:])) ) / np.sqrt(VARn21)  ))
...
#                    return [np.nan]*len(allEVs)
    ...
    if functionName.lower() == 'tversky' or functionName.lower() == '1tversky':
        function = lambda p, param_w: p**np.array(param_w) / (p**np.array(param_w) + (1 - p)**np.array(param_w))**(1 / np.array(param_w))
        pNames = ['temperature']
        p0 = [1]
        return function, pNames, p0
    #%%    
...
def plot_softmax(self, params = [], color='k'):
    ...
    if params != []:
        ...
        vA = value(self.model.exog[:,0:4], params[1:])
        vB = value(self.model.exog[:,4:], params[1:])
    else:
