...
def define_model(decisionFunctions, fixedRange = False, dynamic = False):
    ...
    if len(decisionFunctions) == 3:
        ...
        if decisionFunctions[0] == 'dynamic':
            dynamic = True
            sfm_function, util_function,prob_function = decisionFunctions[1:]
            cumm = False
        else:
            sfm_function, util_function, prob_function, cumm = decisionFunctions
    elif len(decisionFunctions) == 5:
        if decisionFunctions[0] == 'dynamic':
            dynamic = True
            sfm_function, util_function,prob_function, cumm = decisionFunctions[1:]

    elif len(decisionFunctions) == 2:
        if decisionFunctions[0].lower() == 'control':
            sfm_function, util_function, prob_function, cumm = ['random','ev','ev',False]
...
def get_sigmoid(sfm_function, VALUE, dynamic=False):
    ...
    if dynamic == False:
        if len(sig.parameters) > 1:
            if sfm_function.lower() == '2probit':
                SFM = lambda X, params : norm.cdf(x=(VALUE( X[:,:4] , params[2:]) - VALUE(X[:,4:8], params[2:])), loc=params[1], scale=1/params[0])
                sfm_parameters = ['noise', 'side_bias']
                sfm0 = [1, 0]
            elif sfm_function.lower() == '2logit':
                SFM = lambda X, params : 1 / (1 + np.exp( -params[0] * ( (VALUE( X[:,:4] , params[2:]) - VALUE(X[:,4:8], params[2:]) - params[1]) )  ))
                sfm_parameters = ['noise', 'side_bias']
                sfm0 = [1, 0]
                ...
                SFM = lambda X, params : 1 / (1 + np.exp( ( (VALUE( X[:,:4] , params[2:]) - VALUE(X[:,4:8], params[2:]) - params[1]) ) / np.sqrt(VARn21)  ))
                sfm_parameters = ['noise', 'side_bias']
                sfm0 = [1, 0]
            elif sfm_function.lower() == '1probit' or sfm_function.lower() == 'probit'  :
                SFM = lambda X, params : norm.cdf(x=(VALUE( X[:,:4] , params[1:]) - VALUE(X[:,4:8], params[1:])), loc=0, scale=1/params[0])
                sfm_parameters = ['noise']
                sfm0 = [1]
            elif sfm_function.lower() == '1logit' or sfm_function.lower() == 'logit'  :
                SFM = lambda X, params :  1 / (1 + np.exp( -params[0] * ( (VALUE( X[:,:4] , params[1:]) - VALUE(X[:,4:8], params[1:])) )  ))
                sfm_parameters = ['noise']
                sfm0 = [1]
            elif sfm_function.lower() == '1luce':
                SFM = lambda X, params : VALUE( X[:,:4] , params[1:])**params[0] / (VALUE( X[:,:4] , params[1:])**params[0] - VALUE(X[:,4:8], params[1:])**params[0]) 
                sfm_parameters = ['noise']
                sfm0 = [1]
            elif sfm_function.lower() == '2luce':
                SFM = lambda X, params : (VALUE( X[:,:4] , params[2:])**params[0] / (VALUE( X[:,:4] , params[2:])**params[0] - VALUE(X[:,4:8], params[2:])**params[0])) + params[1]
                sfm_parameters = ['noise', 'side_bias']
                sfm0 = [1, 0]
        else:
            if sfm_function.lower() == '2probit':
                SFM = lambda X, params : norm.cdf(x=(VALUE( X[:,:4]) - VALUE(X[:,4:8])), loc= params[1], scale=1/params[0])
                sfm_parameters = ['noise', 'side_bias']
                sfm0 = [1, 0]
            elif sfm_function.lower() == '2logit':
                SFM = lambda X, params : 1 / (1 + np.exp( -params[0] *  (VALUE( X[:,:4] ) - VALUE(X[:,4:8]) - params[1])   ))
                sfm_parameters = ['noise', 'side_bias']
                sfm0 = [1, 0]
            elif sfm_function.lower() == 'random':
                SFM = lambda y, params : np.random.rand(len(y))
                ...
                VARn21 = np.exp(s+(g*time))
                SFM = lambda X, params : 1 / (1 + np.exp( ( (VALUE( X[:,:4] , ) - VALUE(X[:,4:8]) - params[1]) ) / np.sqrt(VARn21)  ))
                sfm0 = [1,0]
            elif sfm_function.lower() == '1probit' or sfm_function.lower() == 'probit' :
                SFM = lambda X, params : norm.cdf(x=(VALUE( X[:,:4]) - VALUE(X[:,4:8])), loc=0, scale=1/params[0])
                sfm_parameters = ['noise']
                sfm0 = [1]
            elif sfm_function.lower() == '1logit' or sfm_function.lower() == 'logit' :
                SFM = lambda X, params :  1 / (1 + np.exp( -params[0] * ( (VALUE( X[:,:4]) - VALUE(X[:,4:8])) )  ))
                sfm_parameters = ['noise']
                sfm0 = [1]
            elif sfm_function.lower() == '1luce':
                SFM = lambda X, params : VALUE( X[:,:4])**params[0] / (VALUE( X[:,:4])**params[0] - VALUE(X[:,4:8])**params[0]) 
                sfm_parameters = ['noise']
                sfm0 = [1]
            elif sfm_function.lower() == '2luce':
                SFM = lambda X, params : (VALUE( X[:,:4] )**params[0] / (VALUE( X[:,:4] )**params[0] - VALUE(X[:,4:8])**params[0])) + params[1]
                sfm_parameters = ['noise', 'side_bias']
                sfm0 = [1, 0]
                
    #%%
    elif dynamic == True:
        if sfm_function.lower() == '1probit' or sfm_function.lower() == 'probit'  :
            def SFM(X, params):
                return norm.cdf(x=(VALUE( X[:,:4], X, params[1:]) - VALUE(X[:,4:8], X, params[1:])), loc=0, scale=1/params[0])
        elif sfm_function.lower() == '1logit' or sfm_function.lower() == 'logit'  :
            def SFM(X, params):
                return 1 / (1 + np.exp( -params[0] * ( VALUE( X[:,:4], X, params[1:]) - VALUE(X[:,4:8], X, params[1:]) )))
        elif sfm_function.lower() == '1luce' or sfm_function.lower() == 'luce'  :
            def SFM(X, params):
                return VALUE( X[:,:4], X, params[1:])**params[0] /  (VALUE( X[:,:4], X, params[1:])**params[0] - VALUE(X[:,4:8], X, params[1:])**params[0] )
        sfm_parameters = ['noise']
        sfm0 = [1]
...
def get_functions(functionName):
    ...
    U(X, params, t)(x[i])
    '''
    #ENDOGENOUS RANDOM UTILITIES
    #%% =============================================================
    #%% Power gamble reference
    if functionName.lower() == 'endog_gamble_pt':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
            ...
            def ref():
                reference = []
                for n,(bb, left, right) in enumerate(zip(both, aRef, bRef)):
                    if bb == True:
                        reference.extend([np.nan])
                    elif left == True:
                        reference.extend([0.0])
                    elif right == True:
                        reference.extend([1.0])
                        
                for n,rr in enumerate(reference):
                    if np.isnan(rr):
                        reference[n] = np.mean([sum(aEVs[n,:]), sum(bEVs[n,:])])
                    elif rr == 0.0:
                        reference[n] = sum(aEVs[n,:])
                    elif rr == 1.0:
                        reference[n] = sum(bEVs[n,:])
                return np.array(reference)
            
            def Utility_dynamic(mm, params, p3 = 0):
                p1,p2 = params
                if p2 < 1 or p1 < 0:
                    return [np.nan] * len(mm)
                return np.where((mm >= p3),
                                    (mm-p3)**p1,
                                    -p2*(p3-mm)**p1)

            def Utility(x, params):
                if x.ndim == 1:
                    return Utility_dynamic(x, params, ref())       
                else:
                    return np.array([Utility_dynamic(xx, params, rr) for xx,rr in zip(np.array(x), ref())]).T
            
            return Utility
                
        pNames, p0 = [['temperature', 'loss_aversion'], [.75,1]]
        return define_utilityFromPast, pNames, p0
    #%% Power safe reference
    if functionName.lower() == 'endog_safe_pt':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
            ...
            def ref():
                ...
                for n,rr in enumerate(reference):
                    if np.isnan(rr):
                        reference[n] = np.mean([aEVs[n,0], bEVs[n,0]])
                    elif rr == 0.0:
                        reference[n] = aEVs[n,0]
                    elif rr == 1.0:
                        reference[n] = bEVs[n,0]
                return np.array(reference)
    ...
    #%% Power left reference
    if functionName.lower() == 'endog_left_pt':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
    ...
    #%% Power right reference
    if functionName.lower() == 'endog_right_pt':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
    ...
    #%% scdf safe reference
    if functionName.lower() == 'endog_safe_scdf':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
        ...
        pNames, p0 = [['temperature'], [1]]
        return define_utilityFromPast, pNames, p0
    
    #%% scdf gamble reference
    if functionName.lower() == 'endog_gamble_scdf':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
    ...
    #DYNAMIC RANDOM UTILITIES
    #%% =============================================================
    #%% ESVT with (n) mean (not RL)
...
#            ref_function = lambda past, p1, t : np.where( (t==0), 0, np.sum([np.mean((p1 * ((1-p1)**(t-1-tau))) * xPast) for tau, xPast in enumerate(past)]) )
            ...
            bEVs = [np.array([gg[0]]) if gg[1] == 0 else gg for gg in options[:, 4:8:2]]
        ...
        pNames, p0 =  [['weighting', 'predisposition', 'n_pastTrials'], [0.5, 1.0, 0.000000000000003]]
        return define_utilityFromPast, pNames, p0
    
    #%%  ESVT EV
    if functionName.lower() == 'esvt_ev':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
            allEVs = np.vstack((aEVs.T, bEVs.T)).T
            allEVs = [ 0 if all(ev == 0.0) else  ev[ev != 0.0] for ev in allEVs]
...
#            allEVs =np.vstack(allEVs)
            ...
            def ref(gamma):
                ...
                def ref_function(past):
                    ref = [ np.mean(allEVs[0])]
                    [ ref.extend([ref[i-1] + np.sum((gamma*( past[i-1] - ref[i-1])))])  for i in range(len(past)) if i != 0 ]
                    return ref
                return ref_function(allEVs)
        ...
        pNames, p0 = [['predisposition', 'learning_EVs'], [1, 0.1]]
        return define_utilityFromPast, pNames, p0
    
    #%% ESVT EV FIXED
    if functionName.lower() == 'esvt_ev_fixed':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
            allEVs = np.vstack((aEVs.T, bEVs.T)).T
...
#            allEVs =np.vstack(allEVs)
            ...
            def ref(params):
                ...
                def ref_function(past):
                    ref = [ params[1]]
                    [ ref.extend([ref[i-1] + np.sum((params[0]*( past[i-1] - ref[i-1])))])  for i in range(len(past)) if i != 0 ]
                    return ref
                return ref_function(allEVs)
        ...
        pNames, p0 = [['predisposition', 'learning_EVs', 'initial_ref'], [1, 0.1, 0.25]]
        return define_utilityFromPast, pNames, p0

    
    #%% ESVT OUTCOME
    if functionName.lower() == 'esvt_outcome':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
            allEVs = np.vstack((aEVs.T, bEVs.T)).T
...
#                past[0] = np.mean(allEVs[0])
        ...
        pNames, p0 = [['predisposition', 'learning_outcomes'], [1,0.1]]
        return define_utilityFromPast, pNames, p0
    
    #%% ESVT OUTCOME FIXED
    if functionName.lower() == 'esvt_outcome_fixed':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
            allEVs = np.vstack((aEVs.T, bEVs.T)).T
...
#                past[0] = np.mean(allEVs[0])
        ...
        pNames, p0 = [['predisposition', 'learning_outcomes', 'initial_ref'], [1,0.1, 0.25]]
        return define_utilityFromPast, pNames, p0

    
    #%% ESVT DOUBLE
    if functionName.lower() == 'esvt_double':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
            allEVs = np.vstack((aEVs.T, bEVs.T)).T
...
#                past[0] = [np.mean(allEVs[0])]
                ...
                def ref_function(past, past2):
                    ref = [np.mean(allEVs[0])]
                    [ ref.extend([ref[i-1] + np.sum((params[0]*( past[i-1] - ref[i-1]))) +  np.sum((params[1]*( past2[i-1] - ref[i-1])))])  for i in range(len(past)) if i != 0 ]
                    return ref
                
#                ref_function = lambda past: np.array([(params[1]*np.mean(p1)) + ((1-params)*np.mean(p2)) for p1, p2 in zip(past, past2)])
                return ref_function(allEVs, outcomes)
        ...
        pNames, p0 = [['predisposition', 'learning_EVs', 'learning_outcomes'], [1, 0, 0]]
        return define_utilityFromPast, pNames, p0

    #%% ESVT DOUBLE FIXED
    if functionName.lower() == 'esvt_double_fixed':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
            allEVs = np.vstack((aEVs.T, bEVs.T)).T
...
#                ref_function = lambda past: np.array([(params[1]*np.mean(p1)) + ((1-params)*np.mean(p2)) for p1, p2 in zip(past, past2)])
        ...
        pNames, p0 = [['predisposition', 'learning_EVs', 'learning_outcomes', 'initial_ref'], [1, 0, 0, 0.25]]
        return define_utilityFromPast, pNames, p0

    #%% SIGMOID EV
    if functionName.lower() == 'sigmoid_ev':
        def define_utilityFromPast(options):
#            rho, gamma = params
#            ref_function = lambda past, p1, t : np.where( (t==0), 0, np.sum([np.mean((p1 * ((1-p1)**(t-1-tau))) * xPast) for tau, xPast in enumerate(past)]) )
                
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
#            aEVs = np.hstack([ 0 if all(ev == 0.0) else  ev[ev != 0.0] for ev in aEVs])
            
            bEVs = np.array([ options[:,4:8][:,i] * options[:,4:8][:,i+1] for i in range(0,np.size(options[:,4:8], 1),2)]).T
#            bEVs = np.hstack([ 0 if all(ev == 0.0) else  ev[ev != 0.0] for ev in bEVs])
            
            allEVs = np.vstack((aEVs.T, bEVs.T)).T
            allEVs = np.vstack([ 0 if all(ev == 0.0) else  ev[ev != 0.0] for ev in allEVs])
            
            
            def ref(params):
                if params[0] < 0 or params[1] < 0:
                    return np.vstack(([np.nan]*len(allEVs),  [np.nan]*len(allEVs))).T
                
                def ref_function(past):
                    ref = [ np.mean(allEVs)]
                    [ ref.extend([ref[i-1] + (params[0]*( np.mean(past[i-1]) - ref[i-1]))])  for i in range(len(past)) if i != 0 ]
                    
                    std = [np.std(allEVs[0])]
                    [ std.extend([ref[i-1] + (params[1]*( np.std(past[i-1]) - ref[i-1]))])  for i in range(len(past)) if i != 0 ]
                    return np.vstack((ref, std)).T
                return ref_function(allEVs)
                        
            def Utility(x, params):
                if x.ndim == 1:
                    variables = ref(params)
                    return norm.cdf(x,  loc=variables[:,0], scale=variables[:,1])   
                else:
                    variables = ref(params)
                    return [norm.cdf(xx,  loc=variables[:,0], scale=variables[:,1]) for xx in np.array(x).T]
            return Utility
        
        pNames, p0 =  [['learning_EVs','scaling'], [ 0.1, 0.1]]
        return define_utilityFromPast, pNames, p0
    
    #%% SIGMOID OUTCOME
    if functionName.lower() == 'sigmoid_outcome':
        def define_utilityFromPast(options):
            aEVs = np.array([ options[:,:4][:,i] * options[:,:4][:,i+1] for i in range(0,np.size(options[:,:4], 1),2)]).T
#            aEVs = np.hstack([ 0 if all(ev == 0.0) else  ev[ev != 0.0] for ev in aEVs])
...
#            bEVs = np.hstack([ 0 if all(ev == 0.0) else  ev[ev != 0.0] for ev in bEVs])
    ...
    #%% SIGMOID DOUBLE
    if functionName.lower() == 'sigmoid_double':
        def define_utilityFromPast(options):
#            rho, gamma = params
#            ref_function = lambda past, p1, t : np.where( (t==0), 0, np.sum([np.mean((p1 * ((1-p1)**(t-1-tau))) * xPast) for tau, xPast in enumerate(past)]) )
...
#            bEVs = np.hstack([ 0 if all(ev == 0.0) else  ev[ev != 0.0] for ev in bEVs])
            ...
            #this is where p1 matters
            def ref(params):
                if  params[0] < 0 or params[1] < 0 or params[0] > 1 or params[1] > 1  or params[2] > 1 or params[2] < 0:
                    return  np.vstack(([np.nan]*len(allEVs),  [np.nan]*len(allEVs))).T
#                past = [ allEVs[:i] for i in range(len(allEVs)) ]
#                past[0] = [np.mean(allEVs[0])]
                ...
                def ref_function(past, past2):
                    ref = [np.mean(allEVs[0])]
                    [ ref.extend([ref[i-1] + (params[0]*( np.mean(past[i-1]) - ref[i-1])) +  (params[1]*( np.mean(past2[i-1]) - ref[i-1]))])  for i in range(len(past)) if i != 0 ]
                    
                    std = [np.std(allEVs[0])]
                    [ std.extend([ref[i-1] + (params[2]*( np.std(past[i-1]) - ref[i-1]))])  for i in range(len(past)) if i != 0 ]
...
#                ref_function = lambda past: np.array([(params[1]*np.mean(p1)) + ((1-params)*np.mean(p2)) for p1, p2 in zip(past, past2)])
        ...
        pNames, p0 =  [['learning_EVs','learning_oucomes', 'scaling'], [ 0, 0, 0]]
        return define_utilityFromPast, pNames, p0
    
    #%% STATIC RANDOM UTILITIES
    #%% =============================================================
    #%% 3 Parameter Power (Prospect Theory)
...
#            return (result - min(result)) / (max(result) - min(result))
    ...
    #%% 2 parameter power (Prospect Theory)
...
#            return (result - min(result)) / (max(result) - min(result))
        ...
        return function, pNames, p0
    #%% 2 parameter power (Prospect Theory)
    if functionName.lower() == '2power_fixed':
        def simple_rho(mm, params):
            p1, p2 = params
            if p2 < 1:
                return [np.nan] * len(mm)
            mm = np.array(mm)
            result = np.ravel(np.where((mm >= 0.4),
                                 (mm-0.4)**p1,
                                -p2*(0.4-mm)**p1))
#            return (result - min(result)) / (max(result) - min(result))
            return result
        function = lambda mm, param_u: simple_rho(mm, param_u) 
        pNames = ['temperature', 'loss_aversion']
        p0 = [1, 1]
        return function, pNames, p0
    #%% 3 parameter CDF of a two-sided power 
    ...
    if functionName.lower() == '2glimcher' or functionName.lower() == 'glimcher' :
        function = lambda mm, param_u: np.where( (param_u[1] > 1 or param_u[1] < 0 or param_u[0] < 1),
                                                np.nan,                
    ...
    if functionName.lower() == 'prelec' or functionName.lower() == '1prelec':
        ...
        p0 = [1]
        return function, pNames, p0
    #%% 2 parameter CDF of a two-sided power function
    if functionName.lower() == '2scdf' or functionName.lower() == 'scdf':
        def sCDF(mm, params):
            Xs = np.atleast_1d(mm) #range position
            inflection, temp = params[1], params[0]
            return np.where((inflection > 1 or inflection < 0 or temp < 0),
                            [np.nan] * len(Xs),
    ...
    if functionName.lower() == 'none':
        function = lambda mm: 1
        pNames = []
        p0 = []
        return function, pNames, p0
    #%% empty
    if functionName.lower() == 'beta':
        function = lambda mm, p1: mm * p1
        pNames = ['beta']
        p0 = [1]
        return function, pNames, p0
...
#     ----------------------------------------------------
        ...
        if sum(uParams == True) == 0 and sum(wParams == True) == 0:
            ...
            if dynamic == True: 
                def VALUE(option, X, params):
                    Utility = U(X)
                    return np.sum([ Utility(option[:,i], params[uParams]) *  W(option[:,i+1], params[wParams]) for i in range(0,np.size(option, 1),2)], 0)                  
                return VALUE
            else:
...
def plot_softmax(self, params = [], color='k'):
    ...
    if params != []:
        ...
        vB = value(self.model.exog[:,4:8], params[2:])
    else:
        value = self.model_parts['value']
        vA = value(self.model.exog[:,0:4])
        vB = value(self.model.exog[:,4:8])
    deltaVal = vA - vB
    chA = self.endog
...
#%%
def plot_gamble(self, trialNo, ax = None):
    
    if ax == None:
        fig, ax = plt.subplots( 1, 1, figsize=( 6,4 ))
        
    gA = self.exog[trialNo, :4]
    gB = self.exog[trialNo, 4:8]
    
    ga_1 = 0.125 ; ga_2 = 0.375
    gb_1 = 0.625; gb_2 = 0.875
    
    
    ax.set_xlim([0.1,1.2]); ax.set_ylim(-0.5,1.5)
    ax.axis('off')
    
    ax.plot([ga_1,ga_1], [0.15, 0.85], color='k')
    ax.plot([ga_2,ga_2], [0.15, 0.85], color='k')
    
    ax.plot([gb_1,gb_1], [0.15, 0.85], color='k')
    ax.plot([gb_2,gb_2], [0.15, 0.85], color='k')
    
    normalizeHeight = lambda x: [((x/1.5) * (0.85 - 0.15)) + 0.15 for _ in range(2)]
    normalizeWidth = lambda x: [ i* (x * (ga_2 - ga_1)) / 2 + (ga_2 - ga_1) for i in [-1, 1]]
    
    ax.plot(normalizeWidth(gA[1]), normalizeHeight(gA[0]), color = 'k')
    ax.plot(normalizeWidth(gA[3]), normalizeHeight(gA[2]), color = 'k')
    
    normalizeWidth = lambda x: [ i* (x * (gb_2 - gb_1)) / 2 + (gb_2 - gb_1) + 0.5 for i in [-1, 1]]
    
    ax.plot(normalizeWidth(gB[1]), normalizeHeight(gB[0]), color = 'k')
    ax.plot(normalizeWidth(gB[3]), normalizeHeight(gB[2]), color = 'k')
    
    ax.text(0.5, 1.2, 'stimuli presented', ha='center', size = 'large')
    
    selected = self.endog[trialNo]
    if selected == 1:
        cc = 'blue'
    else:
        cc = 'red'
    ax.scatter(np.where(selected == 1, (ga_2 - ga_1),(gb_2 - gb_1) + 0.5) , 
               -0.15, color = cc, 
               marker = "^", s = 200)
    
#%%
def get_behaviour(self, trialNo, given_axis = None, xAxis = None, plotPast = True, axis_y_lim = [-0.05, 1.05]):
    '''
    '''
    import matplotlib.pyplot as plt
    ...
    if trialNo > len(self.model.exog):
        trialNo = len(self.model.exog)
    
    if xAxis == None:
        small = 0
        large = max(self.model.exog[:,0])
    else:
        small = xAxis[0]; large = xAxis[1]

    defData = np.linspace(small,large,100)
    norm_def = (defData - small) / (large - small)
    ...
    if any(np.hstack(Utilities) < 0):
        axis_y_lim = [-1.05, 1.05]
        norm_def = defData
        no_EV = True
    
    
#    param_past = int(self.params[1] / np.finfo('double').eps)
    def multipleDistributions(ax, data, utility, i):
        a,b = self.values[i]
#        locations = np.percentile(utility, np.array([10,30,50,70,90]) * large )
        ax.fill_between(data, np.zeros(len(data)), norm.pdf(data, loc=a, scale=noise), color = 'blue', alpha = 0.3)
        ax.fill_between(data, np.zeros(len(data)), norm.pdf(data, loc=b, scale=noise), color = 'red', alpha = 0.3)
        ax.set_yticks([])
        
        yy = ax.get_ylim()[-1]
        
        ax.text(0.05, yy- yy*0.1, 'pChA = ' + str(np.round(self.p_chooseA(self.model.exog)[trialNo], 2)*100) + '%', 
                bbox=dict(facecolor='none', edgecolor='black', pad=2), ha="left", va="top", size="small",color="blue")
        ax.text(0.45, yy + 0.1, str(self.exog[trialNo, :4]) , ha="right", va="bottom", size="small",color="blue")
        ax.text(0.5, yy + 0.1, " & ", ha="center", va="bottom", size="medium")
        ax.text(0.55,yy + 0.1,str(self.exog[trialNo, 4:8]) , ha="left", va="bottom", size="small",color="red")
        
#        ax.set_title(str(self.exog[trialNo, :4]) + ' vs ' + str(self.exog[trialNo, 4:8]) )
        squarePlot(ax)

    if np.size(given_axis) != 3:
...
#    plt.suptitle("Change in 'Utility'")
    ...
    ax[1].set_xlim( [small, large]); ax[1].set_ylim( axis_y_lim)
#    ax[2].set_xlim( [-0.05, 1.05]); ax[2].set_ylim( [-0.05, ])
    plot_gamble(self, trialNo, ax = ax[0])
#    ax[0].set_xlim( [small, large]); ax[0].set_ylim( [-0.05,1.05])
    line, = ax[1].plot( defData, Utilities[trialNo], color='k', alpha = 1, linewidth = 3)
#    line2, = ax[0].plot( defData, normalize(np.gradient(Utilities[trialNo])),
#               color='k', alpha = 1, linewidth = 3)
    
    if plotPast == True:
        ax[1].plot(defData, Utilities[:trialNo].T, color = 'k', alpha = 0.05, linewidth = 0.5)
#        [ax[0].plot(defData, normalize(np.gradient(uu)), color = 'k', alpha = 0.05, linewidth = 0.25) for uu in Utilities[:trialNo]]
    
    text = ax[1].text(small+0.05,.95,'Current Trial: ' + str(trialNo),
             bbox=dict(facecolor='none', edgecolor='black', pad=2))

    optionA, = ax[1].plot(defData, len(norm_def)*[self.values[trialNo,0]], '--', color = 'blue')
    optionB, = ax[1].plot(defData, len(norm_def)*[self.values[trialNo,1]], '--', color = 'red')
    ax[1].text(small + 0.025, self.values[trialNo,0] + 0.015, str(np.round(self.values[trialNo,0], 2)), 
                 ha="left", va="bottom", size="small",color="blue")
    ax[1].text(small+ 0.025, self.values[trialNo,1] + 0.015, str(np.round(self.values[trialNo,1], 2)), 
                 ha="left", va="bottom", size="small",color="red")
    
    multipleDistributions(ax[2], np.linspace(axis_y_lim[0], axis_y_lim[1], 100), Utilities[trialNo], trialNo)

#        ax[0].set_title('last ' + str(param_past) + ' values distribution')
#    ax[1].set_title('normalized value')
#    ax[1].plot(defData, np.median(Utilities.T, axis=1), '--', color='k', linewidth=1)
#    ax[0].plot(defData, normalize(np.gradient(np.median(Utilities.T, axis=1))), '--', color='k', linewidth=1)
#        ax[1].plot(defData, defData, '--', color='k')
#    ax[0].axhline(0, linestyle='--', color = 'k')
    
    ax[1].set_ylabel('subjective value'); ax[1].set_xlabel('reward magnitude')
#    ax[0].set_ylabel('past distribution'); ax[0].set_xlabel('reward magnitude')
        
    ax[2].set_ylabel('value density'); ax[2].set_xlabel('option value')
#    ax[2].set_title('value distributions')
    ax[2].axhline(0, color = 'k')
    
    squarePlot(ax[0]); squarePlot(ax[1])
    
    plt.tight_layout(rect=[0.003, 0, 0.95, 1.0])

   
#%%
def animateBehaviour(self, threeAxis = True, saveName=None, xAxis = None, 
                     axis_y_lim = [-0.05, 1.05], no_EV = False):
    '''
    '''
    import matplotlib.pyplot as plt
    import matplotlib.animation as animation
    ...
    def animate(utility):
        global text, line, line2, i,  optionA, optionB
        text.set_visible(False)
        line.set_alpha(0.05); line.set_linewidth(0.5)
        line2.set_alpha(0.05); line2.set_linewidth(0.5)
        line.set_color('k'); line2.set_color('k')
        prep_axes(ax[1], [small, large], axis_y_lim)
        prep_axes(ax[0], [small, large], axis_y_lim)
        line, = ax[1].plot( defData, utility, color='k', alpha = 1, linewidth = 3)
        line2, = ax[0].plot( defData, normalize(np.gradient(utility)),
                   color='k', alpha = 1, linewidth = 3)
        text = ax[1].text(0.05,.95,'Current Trial: ' + str(i),
                 bbox=dict(facecolor='none', edgecolor='black', pad=2))
        if threeAxis == True:
            optionA.set_data(norm_def, len(norm_def)*[self.values[i,0]])
            optionB.set_data(norm_def, len(norm_def)*[self.values[i,1]])
            multipleDistributions(ax[2], np.linspace(axis_y_lim[0], axis_y_lim[1], 100), utility, i)
        i+=1
        
    def init_animation():
        global line, line2, text, i, optionA, optionB
        i=0
...
#        ax[0].plot(defData, normalize(np.gradient(np.mean(Utilities.T, axis=1))), color='r', linewidth=3)
        if no_EV == False:
            ax[1].plot(defData, defData, '--', color='k')
        ax[0].axhline(0, linestyle='--', color = 'k')
        line, = ax[1].plot( defData, defData, color='k', alpha = 1, linewidth = 3)
        line2, = ax[0].plot( defData, normalize(np.gradient(defData)),
                   color='k', alpha = 1, linewidth = 3)
        optionA, = ax[1].plot(defData, len(norm_def)*[self.values[0,0]], '--', color = 'blue')
        optionB, = ax[1].plot(defData, len(norm_def)*[self.values[0,1]], '--', color = 'red')
        ...
        text = ax[0].text(0.05,.95,'Current Trial: ' + str(i),
                 bbox=dict(facecolor='none', edgecolor='black', pad=2))
        if threeAxis == True:
            multipleDistributions(ax[2], norm_def, np.mean(Utilities, axis=0), i)
            ax[2].set_ylabel('value density'); ax[2].set_xlabel('option value')
            ax[2].set_title('value distributions')
            ax[2].axhline(0, color = 'k')
...
#    defData = np.linspace(small,large,100)
#    Utilities = np.vstack(self.utility(np.tile(defData, (len(self.exog),1)), self.exog)).T
#    Utilities = [ np.vstack(Utilities[:i].T) for i, _ in enumerate(Utilities)]
#    normalize = lambda x :  x / max( x )
#
#    noise = (1/self.params[0]) / np.sqrt(2)
##    param_past = int(self.params[1] / np.finfo('double').eps)
...
##        line.set_color('k'); line2.set_color('k')
#        prep_axes(ax[1], [small, large], [-0.05,1.05])
#        prep_axes(ax[0], [small, large], [-0.05,1.05])
#        ax[1].plot( defData, utility, color='b', alpha = 1, linewidth = 2)
#        ax[0].plot( defData, normalize(np.gradient(utility)),
#                   color='b', alpha = 1, linewidth = 2)
#        ax[1].text(0.05,.95,'Current Trial: ' + str(i),
#                 bbox=dict(facecolor='none', edgecolor='black', pad=2))
#        if threeAxis == True:
...
##        ax[0].set_title('last ' + str(param_past) + ' values distribution')
#        ax[1].set_title('normalized value')
#        ax[1].plot(defData, np.mean(Utilities[-1], axis=1), color='r', linewidth=3)
#        ax[0].plot(defData, normalize(np.gradient(np.mean(Utilities[-1], axis=1))), color='r', linewidth=3)
#        ax[1].plot(defData, defData, '--', color='k')
#        ax[0].axhline(0, linestyle='--', color = 'k')
#        ax[1].plot( defData, defData, color='b', alpha = 1, linewidth = 2)
#        ax[0].plot( defData, normalize(np.gradient(defData)),
#                   color='b', alpha = 1, linewidth = 2)
#        ax[1].set_ylabel('relative value'); ax[1].set_xlabel('reward magnitude')
#        ax[0].set_ylabel('past distribution'); ax[0].set_xlabel('reward magnitude')
#        ax[0].text(0.05,.95,'Current Trial: ' + str(i),
#                 bbox=dict(facecolor='none', edgecolor='black', pad=2))
#        if threeAxis == True:
...
#        plt.rcParams['animation.convert_path'] = r'C:\Program Files\ImageMagick-7.0.8-Q16/magick.exe'
#        writer = animation.ImageMagickFileWriter()
#        ani.save(saveName + '.gif', writer = 'imagemagick', fps=0.1, bitrate=-1)
#
#    #need to make a 3rd subplot where I can show points change in real time.
#    return ani
...
def simulate_CE(self, trials, n=1, mergeBy = 'block',
    ...
    gB = [[gg[0],gg[1]] if gg[1] == 1.0 else gg for gg in self.model.exog[:,4:8].tolist()]
    simulatedTrials['gambleA'] = gA
    simulatedTrials['gambleB'] = gB

    if np.size(params) != 1:
         pchA = self.model_parts['empty functions']['pChooseA'](self.exog, np.array(params))
    else:
         pchA = self.p_chooseA(self.exog)
    simulatedCEs = []
    simulatedOptions = []
    ...
    for _ in tqdm(range(n),  desc='simulating dataset', disable=not plotTQDM):
#        choice = [1 if pp>=rr else 0 for pp, rr in zip(pchA, np.random.rand(len(pchA)))]
        choice = (np.random.rand(len(pchA)) <= pchA) * 1
        
        simulatedTrials['gambleChosen'] = ['A' if cc==1 else 'B' for cc in choice]
        
        accuracy.extend([np.sum(simulatedTrials['gambleChosen'] == trials['gambleChosen']) / len(trials)])
    ...
    indexing = np.argsort(simulatedOptions)
#    jitter = np.vstack([np.random.normal(gg, 0.005, size=len(gg)) for gg in simulatedOptions])

    norm = lambda x: (x - min(self.exog[:,0])) / (max(self.exog[:,0]) - min(self.exog[:,0]))
    
    from scipy import stats
    description = stats.describe(accuracy)
    
    if ax == None:
        ax = plt.gca()
    ax.plot(norm(realEVs), norm(realCEs), color = 'k', marker='o')
    ax.scatter(jitter(norm(simulatedOptions),  0.01), norm(simulatedCEs), color = color, alpha = 0.25)
    ax.plot(norm(realEVs), np.median(norm((np.vstack(simulatedCEs)[:,indexing[0]])), 0),  color = 'dark' + color, marker='o')
    ax.plot(np.linspace(0,1,100), np.linspace(0,1,100), '--', color = 'k', alpha = 0.5, linewidth=0.5)
    ax.set_xlim(left = 0, right = 1)
    ax.set_ylim(bottom = 0, top = 1)
    ax.grid()
    squarePlot(ax)
    ax.set_xlabel('normalized EV') 
    ax.set_ylabel('normalized CE')
    ax.text(0.6, 0.1, 'avg %acc: ' + str(np.round(description.mean, 3)))
    ax.text(0.6, 0.03, 'var: ' + str(np.round(description.variance, 5)))
...
#            print(0)
        ...
        if 'dynamic' in self.model_name.lower():
           fitted.values = np.vstack((fitted.model_parts['value'](fitted.exog[:,:4], fitted.exog),
                           fitted.model_parts['value'](fitted.exog[:,4:8], fitted.exog))).T
           fitted.animateValue = MethodType( animateBehaviour, fitted)
           fitted.get_nValue = MethodType( get_behaviour, fitted)
